...
const CurrentTimeLine = () => {
  ...
  const top = Math.round(
    now.getHours() * CELL_HEIGHT_PX +
      (CELL_HEIGHT_PX / MINUTES_IN_HOUR) * now.getMinutes()
  );

  return (
    <div
      className="absolute z-100 w-full h-[2px] bg-red-500"
  ...
  return (
    <div className="flex flex-col relative overflow-auto h-full">
      ...
      <div className="flex flex-1">
        ...
        <div className="relative grid grid-cols-7 grid-flow-row flex-1">
          <CurrentTimeLine />
          {weekDates.map((hours) => {
            return (
              <div
                className="text-sm text-muted-foreground border-l first:border-l-0 first:bg-muted/50 last:bg-muted/50"
